...
if __name__ == '__main__':
    # scel所在文件夹路径
    in_path = r"/Users/marsli/Desktop/matching/z_sougou/z_scel"
    # 输出词典所在文件夹路径
    out_path = r"/Users/marsli/Desktop/matching/z_sougou/z_text"
    fin = [fname for fname in os.listdir(in_path) if fname[-5:] == ".scel"]
    for f in fin:
        try:
            for word in scel2txt(os.path.join(in_path, f)):
                file_path = (os.path.join(out_path, str(f).split('.')[0] + '.py'))
                # 保存结果
